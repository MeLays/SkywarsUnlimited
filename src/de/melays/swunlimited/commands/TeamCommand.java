...
public class TeamCommand {
	...
	public void onCommand (CommandSender sender , String command , String args[]) {
		...
		helpSender.addAlias("help [page]", "Show this overview", "Use 'help <page>' to get to the next help pages" , "/sw teams help");
		helpSender.addAlias("list", "List created teams", "Lists all teams that have been created!" , "/sw teams list");
		helpSender.addAlias("create <...>", "Create a team", "Create a team that you can add to an cluster\nname - the identifier of the team\n   (wont be shown to users)\ndisplayname - the name shown to the users\n- color - The teamcolor (/sw colorlist)\nmax-players - The max amount of players that can join this team" , "/sw teams create <name> <displayname> <color> <max-players>");
		helpSender.addAlias("remove <name>", "Remove a team", "Remove a team from the teams.yml\nWARNING: Clusters using this team wont load anymore!" , "/sw teams remove <name>");
		...
		if (args.length == 1) {
			...
			if (args.length <= 5) {
				sender.sendMessage(main.getMessageFetcher().getMessage("command_usage", true).replaceAll("%command%", "/sw teams create <name> <displayname> <color> <max-players>"));
				return;
			}
			Color c = new Color(args[4]);
			if (!c.exists()) {
				sender.sendMessage(main.prefix + "This color does not exist! Use /sw colorlist");
				return;
			}
			try {
			...
			if (!main.getMessageFetcher().checkPermission(sender, "swunlimited.setup"))return;
			if (args.length <= 2) {
				sender.sendMessage(main.getMessageFetcher().getMessage("command_usage", true).replaceAll("%command%", "/sw teams remove <name>"));
				return;
			}
			...
			sender.sendMessage(main.getMessageFetcher().getMessage("help.unknown", true).replaceAll("%help%", "/sw teams help"));
		}
