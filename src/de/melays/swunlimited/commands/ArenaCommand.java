...
public class ArenaCommand {
	...
	public void onCommand (CommandSender sender , String command , String args[]) {
		...
		helpSender.addAlias("help [page]", "Show this overview", "Use 'help <page>' to get to the next help pages" , "/sw arenas help");
		helpSender.addAlias("list", "Lists the running games", "Shows all running games" , "/sw arenas list");
		helpSender.addAlias("start <cluster> <player>,...<player>", "Starts a custom game", "Starts a custom game with\ndefault settings" , "/sw arenas start <cluster> <player>,...<player>");
		helpSender.addAlias("stop <cluster>/<player>/<id>/<category>", "Stops an arena", "Stops the arenas:\n - using that cluster\n - with that player\n - with that id\n - in that category" , "/sw arenas stop <cluster>/<player>/<id>/<category>");
		helpSender.addAlias("stopall", "Stops all arenas", "Stops COMPLETELY all arenas." , "/sw arenas stopall");
		...
		if (args.length == 1) {
			...
			if (!main.getMessageFetcher().checkPermission(sender, "swunlimited.setup"))return;
			if (args.length <= 3) {
				sender.sendMessage(main.getMessageFetcher().getMessage("command_usage", true).replaceAll("%command%", "/sw arenas start <cluster> <player>,...<player>"));
				return;
			}
			...
			if (!main.getMessageFetcher().checkPermission(sender, "swunlimited.setup"))return;
			if (args.length != 2) {
				sender.sendMessage(main.getMessageFetcher().getMessage("command_usage", true).replaceAll("%command%", "/sw arenas stopall"));
				return;
			}
			int count = 0;
			...
			if (!main.getMessageFetcher().checkPermission(sender, "swunlimited.setup"))return;
			if (args.length <= 2) {
				sender.sendMessage(main.getMessageFetcher().getMessage("command_usage", true).replaceAll("%command%", "/sw arenas stop <cluster>/<player>/<id>/<category>"));
				return;
			}
			Cluster cluster = null;
			try {
			...
			sender.sendMessage(main.getMessageFetcher().getMessage("help.unknown", true).replaceAll("%help%", "/sw arenas help"));
		}
	}
